...
export function themeToTreeStyles(theme: TreeThemeInput): TreeThemeStyles {
  ...
  if (rawHoverBg != null && rawHoverBg.toLowerCase() !== bgLower) {
    listHoverBg = hoverWouldEraseText(rawHoverBg, sideBarBg, sideBarFg) ? undefined : rawHoverBg
  }
  ...
  const sideBarIsDark = sideBarBgRgb != null ? relativeLuminance(sideBarBgRgb) < 0.5 : isDark
  const result: TreeThemeStyles = {
    ...
    '--trees-theme-list-hover-bg':
      listHoverBg ?? (sideBarIsDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'),
