...
describe("createTextDiff", () => {
  it("creates hunks for modified files", () => {
    ...
    expect(diff.hunks[0]?.lines.some((line) => line.type === "deletion")).toBe(
      true
    )
    expect(diff.hunks[0]?.lines.some((line) => line.type === "addition")).toBe(
      true
    )
  })
...
describe("parseGitPatch", () => {
  ...
  it("drops raw hunk headers if malformed patch text repeats them in hunk bodies", () => {
    ...
    expect(
      files[0]?.hunks[0]?.lines.some((line) => line.text.includes("@@"))
    ).toBe(false)
  })
...
describe("diff projections", () => {
  it("aligns split replacement rows with placeholders", () => {
    ...
    expect(projection.leftRows.some((row) => row.type === "placeholder")).toBe(
      true
    )
    expect(projection.leftRows).toHaveLength(projection.rightRows.length)
    expect(
      projection.leftRows.some((row) => (row.inlineRanges?.length ?? 0) > 0)
    ).toBe(true)
  })
  ...
  it("expands skipped unchanged lines when requested", () => {
    ...
    expect(projection.rows.slice(1, 3).map((row) => row.text)).toEqual([
      "one",
      "two",
    ])
  })
  ...
  it("does not render raw hunk headers if they appear in parsed line content", () => {
    ...
    expect(projection.rows.some((row) => row.text.includes("@@ -255"))).toBe(
      false
    )
  })
})
