...
function pushSplitHunkRows(
  ...
  while (index < lines.length) {
    const nextIndex = pushNextSplitRows(
      leftRows,
      rightRows,
      lines,
      index,
...
function pushSplitExpandedRows(
  ...
  for (let index = 0; index < range.count; index += 1) {
    leftRows.push(
      expandedSideRow(file.oldLines, range.oldStart + index, "old", hunkIndex)
    )
    rightRows.push(
      expandedSideRow(file.newLines, range.newStart + index, "new", hunkIndex)
    )
  }
}
...
function pushNextSplitRows(
  ...
  pushSplitChangeBlock(
    leftRows,
    rightRows,
    changeBlock(lines, index, blockEnd),
    hunkIndex
  )
  return blockEnd
}
...
function splitDeletionRow(
  line: DiffHunkLine | undefined,
  hunkIndex: number
): DiffRenderRow {
  ...
  return renderRowFromLine(
    ...
    "old",
    hunkIndex,
    line.oldInlineRanges
  )
}

function splitAdditionRow(
  line: DiffHunkLine | undefined,
  hunkIndex: number
): DiffRenderRow {
  if (!line) return placeholderRow(hunkIndex)
...
function hunkSeparatorRow(
  ...
  const skippedLines = skippedUnmodifiedLines(
    hunk,
    previousOldEnd,
    previousNewEnd
  )
  ...
  const expandable = skippedRangeAvailable(
    file,
    hunk,
    previousOldEnd,
    previousNewEnd
...
function skippedUnmodifiedLines(
  ...
  return Math.max(
    ...
    hunk.newStart - previousNewEnd - 1,
    0
  )
}
