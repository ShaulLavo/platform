...
export function createSplitProjection(
  ...
  for (const [hunkIndex, hunk] of file.hunks.entries()) {
    const separator = hunkSeparatorRow(file, hunk, previousOldEnd, previousNewEnd, hunkIndex, options);
    ...
    if (separator?.expanded) pushSplitExpandedRows(file, leftRows, rightRows, hunk, previousOldEnd, previousNewEnd, hunkIndex);
...
export function createStackedProjection(
  ...
  for (const [hunkIndex, hunk] of file.hunks.entries()) {
    ...
    if (separator?.expanded) pushStackedExpandedRows(file, rows, hunk, previousOldEnd, previousNewEnd, hunkIndex);
...
function hunkSeparatorRow(
  ...
  return {
    ...
    expandable,
    type: "hunk",
    text: `${action} ${skippedLines} unmodified line${skippedLines === 1 ? "" : "s"}`,
    hunkIndex,
    skippedLines,
  };
}
