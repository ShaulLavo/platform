...
export class DiffView {
  ...
  private createPane(
    ...
    const view = new VirtualizedTextView(host, {
      ...
      gutterContributions: [createDiffGutterContribution(side, () => mountedPane?.rows ?? rows)],
    ...
    const disposeEvents = this.installPaneInteractions(view, () => mountedPane?.rows ?? rows);
  ...
  private refreshSyntaxHighlighting(pane: MountedPane, file: DiffFile): void {
    ...
    void this.applySyntaxHighlighting(pane, file, generation).catch((error: unknown) => {
