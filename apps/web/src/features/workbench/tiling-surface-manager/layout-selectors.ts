...
import { commandSearchText, mergeWindowManagementCommands } from './layout-command-catalog'
import type {
  DropEdge,
  LayoutCommandId,
  LayoutOperation,
...
export function selectCapabilityFilteredCommandTargets(
  ...
  return {
    ...
    minimizableSurfaceIds: visibleSurfaces
      .filter((surface) => surface.capabilities.canMinimize)
...
function selectMruSurfaceFallback(layout: WorkspaceLayout): Surface | null {
  const minimizedSurfaceIds = new Set(layout.rail.minimizedSurfaceIds)

  for (const surfaceId of layout.mruSurfaceIds) {
    const surface = layout.surfacesById[surfaceId]
    if (!surface || minimizedSurfaceIds.has(surfaceId)) continue
...
function activeCapabilityDisabledReason(layout: WorkspaceLayout, operation: LayoutOperationType) {
  ...
  if (operation === 'minimizeSurface' && activeSurface && !activeSurface.capabilities.canMinimize) {
