...
export function WorkbenchFixtureSurface({
  active,
  surface,
  visible,
}: WorkbenchSurfaceRendererProps) {
  return (
    <section
    ...
    >
      <div className='border-border/70 bg-muted/20 flex min-h-0 flex-1 items-center justify-center border-t p-6'>
        <div className='border-border/70 bg-background/80 grid w-full max-w-md gap-4 rounded-md border p-5 shadow-sm backdrop-blur-sm'>
          <div className='flex min-w-0 items-center gap-3'>
            <div className='bg-muted text-muted-foreground flex size-9 shrink-0 items-center justify-center rounded-md'>
              <WorkbenchSurfaceIcon className='size-5' type={surface.type} />
            </div>
...
function surfaceStateLabel(surface: WorkbenchSurfaceRendererProps['surface']) {
