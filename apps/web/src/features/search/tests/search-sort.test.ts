...
describe('search sort', () => {
  it('sorts file names with numeric chunks', () => {
    ...
    expect(names.sort(compareSearchFileNames)).toEqual(['file-1.ts', 'file-2.ts', 'file-10.ts'])
  })
  ...
  it('sorts path components case-insensitively before filenames', () => {
    const paths = ['repo/Src/file-10.ts', 'repo/docs/file-1.ts', 'repo/src/file-2.ts']
  ...
  it('keeps collator ordering for filenames outside the fast path', () => {
    ...
    expect(names.toSorted(compareSearchFileNames)).toEqual(
      names.toSorted((left, right) => collator.compare(left, right)),
    )
  })
})
