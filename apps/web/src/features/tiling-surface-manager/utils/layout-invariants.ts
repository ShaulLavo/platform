import { builtInWindowManagementCommands } from '@/features/tiling-surface-manager/utils/layout-command-catalog'
import type {
  LayoutNode,
  LayoutNodeId,
  LayoutSplitAxis,
...
function checkMinimizedSurfaces(
  layout: WorkspaceLayout,
  references: readonly VisibleSurfaceReference[],
  violations: LayoutInvariantViolation[],
) {
  ...
  for (const surfaceId of layout.rail.minimizedSurfaceIds) {
    if (!visibleSurfaceIds.has(surfaceId)) continue

    pushViolation(
      violations,
      'minimized-surface-visible',
      `Minimized surface ${surfaceId} is visible.`,
      {
        surfaceId,
      },
    )
...
function checkRailReferences(layout: WorkspaceLayout, violations: LayoutInvariantViolation[]) {
  checkRailSurfaceIds(layout, layout.rail.minimizedSurfaceIds, violations)
