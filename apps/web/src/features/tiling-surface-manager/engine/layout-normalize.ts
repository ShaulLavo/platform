...
type SplitChildNormalization = {
  readonly result: NormalizedNodeResult
  readonly size: number
}
...
function collectSplitChildren(
  ...
  const children = normalizedSplitChildren(layout, node, seenNodeIds)

  for (const child of children) {
    if (!child) continue

    visibleWindowIds.push(...child.result.visibleWindowIds)
    addNormalizedChild(entries, nodesById, child.result, node.axis, child.size)
  }
...
function repairWindows(layout: WorkspaceLayout) {
  const windowsById: Record<string, WorkbenchWindow> = {}

  for (const window of Object.values(layout.windowsById)) {
    const repairedWindow = repairWindow(window, layout.surfacesById)
...
function repairWindow(
  ...
  const surfaceIds = uniqueSurfaceIds(window.surfaceIds).filter((surfaceId) =>
    Boolean(surfacesById[surfaceId]),
  )
...
function addRailSurface(
  ...
  const surfaceIds = rail[key] ?? []
  if (surfaceIds.includes(surfaceId)) return rail
