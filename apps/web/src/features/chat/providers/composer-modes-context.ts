 ...
 * one is both a draft override for the next turn and a session-level change, so
 * the sidebar and any second client see the same mode this composer shows.
 */
export type ChatComposerModes = {
  /** True once the session command is accepted; the local pick stands either way. */
