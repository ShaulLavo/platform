...
test('the shell snapshot loads over HTTP with contract-shaped dates', async ({ client }) => {
  ...
  const snapshot = await fetchOrchestrationShellSnapshotHttp()
...
test('the thread detail snapshot loads over HTTP with contract-shaped dates', async ({
  ...
  const snapshot = await fetchOrchestrationThreadDetailSnapshotHttp(THREAD_ID)
...
test('the local chat environment reads thread detail over HTTP', async ({ client }) => {
  await seedThread(client)

  const snapshot = await createLocalChatEnvironment().threadDetailSnapshot(THREAD_ID)
...
test('an HTTP snapshot failure classifies the same way the RPC error did', async ({ client }) => {
  ...
  const failure = await rejection(fetchOrchestrationThreadDetailSnapshotHttp(missing))
