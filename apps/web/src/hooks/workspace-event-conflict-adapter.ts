...
export async function notifyRenamedFilesystemConflict(
  ...
  const remoteFile = await context.fetchFile(
    remotePath,
...
export function dismissFilesystemConflicts(
  conflictStore: EditorConflictStoreApi
) {
...
async function applyLocalConflict(
  ...
  const file = await context.fetchFile(
    conflict.remotePath,
    new AbortController().signal
  )
...
function discardResolvedEditorFile(
  path: string,
  context: WorkspaceConflictContext
) {
