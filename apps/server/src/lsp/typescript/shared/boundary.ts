...
export function fileNameForUri(
  ...
  const workspaceFileName = documentUriToWorkspaceFileName(
    ctx.workspaceRoot,
    uri
  )
...
export function documentUriForFileName(
  ...
  const normalized = normalizeNativePath(fileName)
  if (!isInsidePath(ctx.workspaceRoot, normalized))
    return fileNameToDocumentUri(normalized)
...
function documentUriToWorkspaceFileName(
