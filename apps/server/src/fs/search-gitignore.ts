...
export async function workspaceGitIgnoreMatcher(paths: WorkspacePaths): Promise<GitIgnoreMatcher> {
...
function gitIgnoreRulesIgnorePath(rules: readonly GitIgnoreRule[], relativePath: string) {
...
function gitIgnoreRuleMatches(rule: GitIgnoreRule, relativePath: string) {
  ...
  return relativePath.split('/').some((segment) => globPatternMatches(segment, rule.pattern))
}
