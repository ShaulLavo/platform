...
| **No file-tree drag into the composer** — `handleComposerDrop` accepts image `Files` only; a dragged tree node is ignored. Needs the insert seam. `composer-file-tree-mention-drag`                                                                                                                                                                          | open    | low    | M      | `apps/web/src/features/chat/components/chat-input.tsx:287,330,389`                          | `components/chat/composerMentionDrag.ts` |
...
| **`draftPromotion` / `markDraftPromotion` are dead store fields** — zero non-test callers now that the single-dispatch draft path made them unnecessary. Delete them. `thread-lifecycle-draft-entity-and-promotion`                                                                                                                                          | partial | low    | S      | `apps/web/src/features/chat/state/chat-input-draft-store.ts:47,67,116`                      | —                                        |
...
| **No commit/push/PR stacked action with progress and hook streaming** — a long pre-commit hook still looks like a hung button. `git-stacked-actions`                                                                                                                                                                                                                                                                                                                                                                                                                                            | open     | medium | XL     | `apps/server/src/git/service.ts`, `routes.ts`, `apps/web/src/features/git/components/header.tsx`                                | `components/GitActionsControl.tsx`                |
| **No PR affordances from chat** — scope to GitHub via `gh` only. `git-pr-affordances-from-chat`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | open     | medium | XL     | `packages/contracts/src/git.ts`, `apps/server/src/git/service.ts`                                                               | `sourceControl/PrTemplateDetection.ts`            |
...
| **Project scripts do not exist** — no `ProjectScript` type, no `scripts_json`, no meta-update field. `contracts-project-scripts`, `persistence-projects-scripts-column`, `orch-project-scripts`                                                                                                                                                                                                                                                                                                                                                                                                 | open     | medium | M      | `packages/contracts/src/chat-model.ts`, `apps/server/src/db/schema.ts`                                                          | `packages/contracts/src/orchestration.ts:184-225` |
