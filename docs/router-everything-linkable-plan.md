...
Settings is a registry-driven page (`features/settings/components/page.tsx`), opened as a `settings:` editor tab when a folder is open and as a dialog when there is none — see `docs/settings-architecture-plan.md`. `?settings=` therefore selects a **category** on that page rather than naming a section component; the section components it used to name are gone. It stays a param rather than a path for the reason below: a real route unmounts the workspace behind it. With no folder open the same param opens the folderless dialog, which is why `/~-?settings=providers` still resolves.
...
`?side`, `?bottom`, `?settings=`. Cheap single fields that exercise the retention rule before anything expensive depends on it. Settings v1 selects the named category on the settings page — a small real addition, not free.
...
| **Settings as a path**                                                                                 | Addressable as `?settings=`, never as `/settings`. A real route unmounts the workspace behind it, killing live terminal sockets and editor DOM, and forces the reference's guess-the-exit problem (`refs/.../routes/settings.tsx:40-46`). The param now selects a category on the settings page — an editor tab with a folderless dialog fallback — rather than opening an overlay panel; the conclusion survives the change, the mechanism it described does not. |
