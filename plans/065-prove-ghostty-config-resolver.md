...
by construction. Any build-consumed submodule, downloaded dependency, SDK, generated resource
materialized before its consuming recorded command, or other external byte outside the top-level blob
set must be a separate origin/revision-or-URL/length/SHA-256 entry in `proofRecipeSha256`; unresolved
external input identity makes the proof `FAIL`. A transitive output produced and consumed inside one
recorded command remains internal to that command. Freeze tests show that two checkouts with
different Git metadata, mtimes, and untracked caches produce the same digest, while a tracked path,
mode, blob byte, symlink target, or gitlink change changes it. Plan 066 must reuse this exact versioned
algorithm and vectors.
...
ID, and one of the same acquisition variants. A generated byte recorded as an external input must
identify all source records and the exact generating argv; it cannot be represented only by an output
pathname. Arrays sort by role then ID/name using unsigned UTF-8 bytes; environment names are unique
and byte-sorted. Strings, counts, paths, byte lengths, versions, URLs, and hashes use the bounds later
required by the strict evidence verifier. No ambient path, floating version, mutable URL, free-form
command string, or inherited environment entry is allowed.
...
The proof runner loads this recipe, recomputes the upstream tree, verifies every recipe-bound
tool/input byte against its record before use, invokes each distinct recorded generation argv and
the recorded outer-build argv under the recorded build environment, and invokes the recorded strip
argv. The fixed themes-archive extraction is orchestration outside those native command fields: the
...
`-j1 --seed 0 -fincremental`. The build copies the verified Zig executable to the target's
...
The proof runner exposes one reviewed interface for CI and local reproduction. For example, after
setting the workflow identity environment, the Darwin arm64 pair is:

```bash
bun scripts/config-resolver-proof/run.ts --mode build --target darwin-arm64 \
  --upstream /tmp/plan-065-upstream --zig /tmp/plan-065-zig/zig \
  --zig-archive /tmp/plan-065-zig.tar.xz --themes-archive /tmp/plan-065-themes.tgz \
  --output /tmp/plan-065-darwin-arm64-output \
  --evidence /tmp/plan-065-darwin-arm64-build.json
bun scripts/config-resolver-proof/run.ts --mode verify --target darwin-arm64 \
  --upstream /tmp/plan-065-upstream --zig /tmp/plan-065-zig/zig \
  --zig-archive /tmp/plan-065-zig.tar.xz --themes-archive /tmp/plan-065-themes.tgz \
  --output /tmp/plan-065-darwin-arm64-output \
  --evidence /tmp/plan-065-darwin-arm64-verify.json
```

These operator input/output locations are illustrative and are separate from the canonical
`/private/tmp/ghostty-config-resolver-proof-build-v1` native build root. Record the exact equivalent
commands using the other three target keys. `verify` rejects a nonmatching host, relocates the
bundle, removes Ghostty/Zig/source from its runtime `PATH`, and runs the complete fixture suite.
